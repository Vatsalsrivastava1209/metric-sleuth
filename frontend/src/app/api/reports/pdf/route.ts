...
export async function GET(request: Request) {
  ...
  const upstream = await fetch(
    buildBackendApiUrl(`/api/v1/export/pdf/${encodeURIComponent(reportId)}?${upstreamParams.toString()}`),
    {
      headers: {
  ...
  const exportFallback = upstream.headers.get('x-export-fallback')
  ...
  if (exportFallback) {
    response.headers.set('X-Export-Fallback', exportFallback)
  }
